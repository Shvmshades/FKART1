class BillingController < ApplicationController
  ...
  def create_card
    respond_to do |format|
      if Current.user.stripe_id.nil?
        debugger
        customer = Stripe::Customer.create({"email": Current.user.email})
        #here we are creating a stripe customer with the help of the Stripe library and pass as parameter email. 
        Current.user.update(:stripe_id => Current.user.id)
      ...
      customer = Stripe::Customer.new Current.user.stripe_id
